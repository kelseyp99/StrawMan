...
// RelatedLogEntry (no memo)
function RelatedLogEntry({
  log,
  description,
  onDescriptionChange,
  onCategoryChange,
...
const MainComponent: React.FC = () => {
  ...
  const filteredData = useCallback(() => {
    ...
    // Only filter/clean Activity Log table, always show all for Discussions and Categories
    if (currentTable.name === 'Activity Log Data' || currentTable.name === 'Activities') {
      const seen = new Set();
      const uniqueRows = sorted.filter((row) => {
        const { id, timestamp, rawTimestamp, description, category, ...rest } = row;
        const normDesc = (description || '').replace(/\s+/g, ' ').trim().toLowerCase();
        const normCat = (category || '').replace(/\s+/g, ' ').trim().toLowerCase();
        if (!normDesc || !normCat || normCat === 'uncategorized') return false;
        const key = JSON.stringify({ ...rest, description: normDesc, category: normCat });
      ...
      if (!hasFilters) return uniqueRows;
      ...
      // For Discussions and Categories, just apply filters (no deduplication or hiding)
      const hasFilters = Object.values(filters).some((value) => value && value.trim());
  ...
  }, [tables, filters, sortedData, currentTableIndex]);
  ...
  const handleEdit = useCallback(
    async (
      ...
      if (tableName === 'Categories') {
        // Always refresh categories before opening modal to ensure latest data
        await loadCategories();
        const table = tables.find((t) => t.name === tableName);
        const categoryItem = table?.data.find((i) => i.id === itemId);
        if (categoryItem) {
          console.log('Opening category edit modal for:', itemId, categoryItem.name);
  ...
  const handleDeleteActivityLog = useCallback(async (id: string) => {
    try {
      console.log('===DELETE_DEBUG=== Deleting ActivityLog id:', id);
      ...
      console.log('===DELETE_DEBUG=== Calling fetchData after delete');
      await fetchData();
      console.log('===DELETE_DEBUG=== fetchData complete after delete');

      // Check if the deleted ID is still present in the data
      if (typeof filteredData === 'function') {
        const dataAfter = filteredData();
        const stillThere = dataAfter.some((item) => item.id === id);
        if (stillThere) {
          console.error('===DELETE_DEBUG=== ERROR: Deleted ActivityLog id still present after fetch:', id);
          Alert.alert('Delete Failed', 'The entry still appears after deletion.');
        } else {
          console.log('===DELETE_DEBUG=== SUCCESS: Deleted ActivityLog id is gone:', id);
        }
      }
    } catch (error) {
      console.error('===DELETE_DEBUG=== Error deleting activity log:', error);
        ...
        <View style={styles.tableContainer}>
          ...
          >
            <View style={[styles.modalOverlay, { zIndex: 1000, pointerEvents: 'auto' }]}> 
              <ScrollView style={styles.modalContainer} pointerEvents="auto">
                <Text style={styles.modalTitle}>
                  Edit {typeof editTableName === 'string' && editTableName.trim().length > 0 ?
                    (editTableName.trim().toLowerCase() === 'entry' ? 'Entry' : editTableName)
                    : 'Entry'}
                </Text>
                ...
                <View style={styles.modalButtons}>
                  ...
                  >
                    <Text style={styles.modalButtonText}>Edit Categories</Text>
                  </TouchableOpacity>
                </View>
              </ScrollView>
            </View>
